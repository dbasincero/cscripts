--==========================================================================
-- Script    : hf_latency_internal_query_5.sql
...
--==========================================================================
-- hf_latency_internal_query_5.sql: called by hf_latency_range_iod.sql, hf_latency_range_iod_extended.sql, hf_latency_snapshot.sql, hf_latency_snapshot_extended.sql
PRO 
...
SELECT LAG(s.snap_timestamp) OVER (PARTITION BY s.snap_type, s.sid, s.con_id, s.sql_id ORDER BY s.snap_timestamp) AS begin_timestamp,
 ...
 WHERE s.snap_timestamp >= TO_TIMESTAMP('&&cs_sample_time_from.', '&&cs_datetime_full_format.') - INTERVAL '2' MINUTE -- needed by hf_latency_range.sql because of LAG functions above
   AND s.snap_timestamp <= TO_TIMESTAMP('&&cs_sample_time_to.', '&&cs_datetime_full_format.') + INTERVAL '1' SECOND -- needed by hf_latency_snapshot.sql
...
SELECT MIN(d.begin_timestamp) AS begin_timestamp,
 ...
 WHERE d.seconds > 1 -- avoid snaps less than 1 sec appart
   ...
   AND d.end_timestamp <= TO_TIMESTAMP('&&cs_sample_time_to.', '&&cs_datetime_full_format.') + INTERVAL '1' SECOND -- needed by hf_latency_snapshot.sql
 GROUP BY
       d.con_id,
       d.sqlid,
       d.sql_id,
