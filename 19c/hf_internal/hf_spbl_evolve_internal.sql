--==========================================================================
-- Script    : hf_spbl_evolve_internal.sql
...
BEGIN
  ...
  IF l_plans > 0 THEN 
    ...
    DBMS_SPM.alter_sql_plan_baseline (
      ...
      attribute_value => 'hf_spbl_evolve.sql SQL_ID='||p_sql_id||' PHV='||l_current_plan_hash_value||' STG='||k_staging_name||' USR=&&who_am_i.'
    );
  END IF;
  --
  -- ****************************************************************************************************************************************************************************************************************
  ...
  LOOP
    ...
    DBMS_SQLTUNE.import_sql_profile(
      ...
      name        => k_staging_name,
      description => 'hf_spbl_evolve.sql SQL_ID='||p_sql_id||' PHV='||i.plan_hash_value||' USR=&&who_am_i.',
      category    => 'DEFAULT',
      validate    => TRUE,
      replace     => TRUE
    );
  ...
  LOOP
    ...
    DBMS_SQLTUNE.import_sql_profile(
      ...
      profile     => l_profile_attr,
      name        => k_staging_name,
      description => 'hf_spbl_evolve.sql SQL_ID='||p_sql_id||' PLAN_ID='||i.plan_id||' USR=&&who_am_i.',
      category    => 'DEFAULT',
      validate    => TRUE,
      replace     => TRUE
    );
  ...
  LOOP
    ...
    $THEN
      DBMS_SQLDIAG_INTERNAL.i_create_patch (
        ...
        name        => k_staging_name, 
        description => 'hf_spbl_evolve.sql /*+ '||i.cbo_hints||' */ USR=&&who_am_i.'
      ); -- 12c
    $ELSE
      l_name := 
  ...
  DBMS_SPM.create_evolve_task (
    ...
    description => 'hf_spbl_evolve.sql SQL_ID='||p_sql_id||' USR=&&who_am_i.'
  );
  ...
  DBMS_SPM.execute_evolve_task (
    ...
    execution_desc  => 'hf_spbl_evolve.sql SQL_ID='||p_sql_id||' USR=&&who_am_i.'
  );
  ...
  IF l_evolved_plans > 0 THEN
    ...
    DBMS_SPM.alter_sql_plan_baseline (
      ...
      plan_name       => l_current_plan_name,
      attribute_name  => 'description',
      attribute_value => 'hf_spbl_evolve.sql SQL_ID='||p_sql_id||' PHV='||l_current_plan_hash_value||' STG='||k_staging_name||' USR=&&who_am_i. DISABLED'
    );
    ...
    LOOP
      ...
      IF i.executions = 0 OR i.et_ms_per_exec > l_current_et_ms_per_exec THEN
        ...
        DBMS_SPM.alter_sql_plan_baseline (
          ...
          plan_name       => i.sql_plan_baseline,
          attribute_name  => 'description',
          attribute_value => 'hf_spbl_evolve.sql SQL_ID='||p_sql_id||' PHV='||i.plan_hash_value||'EXEC='||i.executions||' MS_PER_EXEC:'||ROUND(i.et_ms_per_exec, 3)||' USR=&&who_am_i. DISABLED'
        );
      ...
      ELSE -- evolved plan had some executions and its performance is better than current
        ...
        DBMS_SPM.alter_sql_plan_baseline (
          ...
          plan_name       => i.sql_plan_baseline,
          attribute_name  => 'description',
          attribute_value => 'hf_spbl_evolve.sql SQL_ID='||p_sql_id||' PHV='||i.plan_hash_value||' EXECS='||i.executions||' MS_PER_EXEC:'||ROUND(i.et_ms_per_exec, 3)||' USR=&&who_am_i. VERIFIED EVOLVED'
        );
    ...
    IF l_verified_plans = 0 THEN
      ...
      DBMS_SPM.alter_sql_plan_baseline (
        ...
        plan_name       => l_current_plan_name,
        attribute_name  => 'description',
        attribute_value => 'hf_spbl_evolve.sql SQL_ID='||p_sql_id||' PHV='||l_current_plan_hash_value||' STG='||k_staging_name||' USR=&&who_am_i. RE-ENABLED'
      );
    ...
    LOOP
      ...
      DBMS_SPM.alter_sql_plan_baseline (
        ...
        plan_name       => i.plan_name,
        attribute_name  => 'description',
        attribute_value => 'hf_spbl_evolve.sql SQL_ID='||p_sql_id||' USR=&&who_am_i. UNVERIFIED DISABLED'
      );
