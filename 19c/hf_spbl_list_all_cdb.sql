--==========================================================================
-- Script    : hf_spbl_list_all_cdb.sql
-- Versao    : Oracle Database 19c (19.x)
-- Uso       : Lista todos os SQL Plan Baselines para alguma string de SQL
--             Text no CDB
...
----------------------------------------------------------------------------------------
--
-- File name:   hf_spbl_list_all_cdb.sql
--
-- Purpose:     List all SQL Plan Baselines for some SQL Text string on CDB
--
...
--
DEF cs_script_name = 'cs_spbl_list_all_cdb';
--
...
--
PRO
PRO SQL PLAN BASELINES - LIST (cdb_sql_plan_baselines)
PRO ~~~~~~~~~~~~~~~~~~~~~~~~~
WITH
...
SELECT /*+ MATERIALIZE NO_MERGE */
       con_id, sql_plan_baseline, COUNT(*) AS child_cursors
  FROM v$sql
 WHERE sql_plan_baseline IS NOT NULL
   AND ROWNUM >= 1
 GROUP BY
       con_id, sql_plan_baseline
)
