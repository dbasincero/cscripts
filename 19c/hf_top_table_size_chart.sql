--==========================================================================
-- Script    : hf_top_table_size_chart.sql
-- Versao    : Oracle Database 19c (19.x)
-- Uso       : Top Tabelas de PDB (gráfico de série temporal)
-- Variaveis  : &&cs_con_name, &&cs_sample_time_from, &&cs_sample_time_to
-- Pre-req    : conexao com privilegio de DBA (acessa views V$/DBA_/CDB_).
--==========================================================================
----------------------------------------------------------------------------------------
--
-- File name:   hf_top_table_size_chart.sql 
--
-- Purpose:     Top PDB Tables (time series chart)
--
...
--              SQL> @hf_top_table_size_chart.sql
--
...
SELECT /*+ MATERIALIZE NO_MERGE */
 ...
 WHERE snap_time = (SELECT MAX(snap_time) FROM &&cs_tools_schema..dbc_segments WHERE snap_time >= TO_DATE('&&cs_sample_time_from.', '&&cs_datetime_full_format.') AND snap_time < TO_DATE('&&cs_sample_time_to.', '&&cs_datetime_full_format.'))
   ...
   AND owner IN (SELECT /*+ OPT_PARAM('_px_cdb_view_enabled' 'FALSE') */ username FROM cdb_users WHERE con_id = &&cs_con_id. AND oracle_maintained = 'N' AND username NOT LIKE 'C##%')
 GROUP BY
       owner,
       segment_name,
       CASE 
