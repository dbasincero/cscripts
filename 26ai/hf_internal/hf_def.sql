...
--             para detalhes).
-- Variaveis  : &&cs_con_name, &&cs_dbid, &&cs_reference
-- Seguranca  : executa comando de SO (host/!) — confira o comando antes de rodar; usa EXECUTE IMMEDIATE (SQL dinâmico); gera arquivo via SPOOL
...
DEF spool_id_chart_footer_script = 'hf_null.sql';
...
SELECT CASE WHEN '&&cs_odis.' = 'Y' THEN '--' END AS cs_skip,
       CASE WHEN '&&cs_odis.' = 'Y' THEN 'hf_spbl_internal_list_simple.sql' ELSE 'hf_spbl_internal_list_debug.sql' END AS list_sqlbaseline_script,
       CASE WHEN '&&cs_odis.' = 'Y' THEN 'hf_null.sql' ELSE 'hf_internal_list_dg_members.sql' END AS list_dg_members_script,
       CASE WHEN '&&cs_odis.' = 'Y' THEN 'hf_null.sql' ELSE 'hf_set_container_to_cdb_root.sql' END AS cs_set_container_to_cdb_root,
       CASE WHEN '&&cs_odis.' = 'Y' THEN 'hf_null.sql' ELSE 'hf_set_container_to_curr_pdb.sql' END AS cs_set_container_to_curr_pdb,
       ...
       CASE WHEN '&&cs_odis.' = 'Y' THEN 'hf_null.sql' ELSE 'hf_spbl_create_pre.sql' END AS cs_spbl_create_pre,
       CASE WHEN '&&cs_odis.' = 'Y' THEN 'hf_null.sql' ELSE 'hf_spbl_create_post.sql' END AS cs_spbl_create_post,
       CASE WHEN '&&cs_odis.' = 'Y' THEN 'hf_null.sql' ELSE 'hf_spbl_validate.sql' END AS cs_spbl_validate,
       CASE WHEN '&&cs_odis.' = 'Y' THEN 'hf_null.sql' ELSE 'hf_list_cbo_hints.sql' END AS cs_list_cbo_hints,
       CASE WHEN '&&cs_odis.' = 'Y' THEN 'hf_null.sql' ELSE 'hf_list_cbo_hints_b.sql' END AS cs_list_cbo_hints_b,
       CASE WHEN '&&cs_odis.' = 'Y' THEN 'hf_null.sql' ELSE 'hf_zapper_managed.sql' END AS cs_zapper_managed
  FROM DUAL
/
--
SELECT CASE WHEN COUNT(*) > 0 THEN 'hf_spbl_internal_stgtab_baseline.sql' ELSE 'hf_null.sql' END AS stgtab_sqlbaseline_script
  FROM dba_tables
 WHERE owner = UPPER('&&cs_stgtab_owner.')
   AND table_name = UPPER('&&cs_stgtab_prefix._stgtab_baseline')
/
--
SELECT CASE WHEN COUNT(*) > 0 THEN 'hf_sprf_internal_stgtab_sqlprofile.sql' ELSE 'hf_null.sql' END AS stgtab_sqlprofile_script
  FROM dba_tables
 WHERE owner = UPPER('&&cs_stgtab_owner.')
   AND table_name = UPPER('&&cs_stgtab_prefix._stgtab_sqlprof')
/
--
SELECT CASE WHEN COUNT(*) > 0 THEN 'hf_spch_internal_stgtab_sqlpatch.sql' ELSE 'hf_null.sql' END AS stgtab_sqlpatch_script
  FROM dba_tables
 WHERE owner = UPPER('&&cs_stgtab_owner.')
...
--
--
-- NOTA DE VERSAO (26ai): este conjunto espera i.version_full iniciando com '26.'.
--   - Em 26ai (linhagem 23ai) a arquitetura non-CDB e DESUPORTADA; todos os
--     scripts ja assumem CDB/PDB (v$containers, con_id, cdb_*), portanto compativeis.
--   - version_full continua valido; views V$/DBA_/CDB_ usadas seguem presentes.
--
...
@@hf_sample_time_boundaries.sql
@@hf_snap_id_from_and_to.sql
--
/****************************************************************************************/
--
...
SELECT CASE WHEN COUNT(*) > 0 THEN 'hf_zapper_19_actions.sql' ELSE 'hf_null.sql' END AS zapper_19_actions_script
  FROM dba_tables
 WHERE owner = UPPER('&&cs_stgtab_owner.')
...
SELECT CASE WHEN COUNT(*) > 0 THEN 'hf_oem_me_sqlperf.sql' ELSE 'hf_null.sql' END AS oem_me_sqlperf_script
  FROM dba_tables
 WHERE owner = UPPER('&&cs_stgtab_owner.')
...
SELECT CASE WHEN COUNT(*) > 0 THEN 'hf_blackout_internal.sql' ELSE 'hf_null.sql' END AS blackout_dates_script
  FROM dba_tables
 WHERE owner = UPPER('&&cs_stgtab_owner.')
...
SELECT '&&cs_reference.'||
  ...
  FROM DUAL
/
SPO /tmp/hf_default_reference.sql;
