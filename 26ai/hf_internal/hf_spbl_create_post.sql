--==========================================================================
-- Script    : hf_spbl_create_post.sql
...
BEGIN
  ...
  LOOP
    ...
    l_description := TRIM(i.description||' hf_spbl_create.sql ORA-13831 DISABLED='||TO_CHAR(SYSDATE, '&&cs_datetime_full_format.'));
...
BEGIN 
  ...
  LOOP 
    l_plans := DBMS_SPM.alter_sql_plan_baseline(sql_handle => i.sql_handle, plan_name => i.plan_name, attribute_name => 'ENABLED', attribute_value => 'NO'); 
    l_description := i.description||' hf_spbl_create.sql ORA-06512 DISABLED='||TO_CHAR(SYSDATE, '&&cs_datetime_full_format.');
