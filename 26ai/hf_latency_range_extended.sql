--==========================================================================
-- Script    : hf_latency_range_extended.sql
-- Versao    : Oracle Database 26ai (26.x)
-- Uso       : Latência de SQL para um intervalo de tempo (tempo decorrido
--             por execução) (AWR) - Granularidade 15m - Estendido
-- Atalhos    : lre.sql
-- Variaveis  : &&cs_sample_time_from, &&cs_sample_time_to
-- Pre-req    : conexao com privilegio de DBA (acessa views V$/DBA_/CDB_).
--==========================================================================
----------------------------------------------------------------------------------------
--
-- File name:   lre.sql | hf_latency_range_extended.sql
--
-- Purpose:     SQL latency for a time range (elapsed time over executions) (AWR) - 15m Granularity - Extended
--
-- Author:      Carlos Sierra
--
-- Version:     2022/08/06
--
-- Usage:       Execute connected to PDB or CDB
--
-- Example:     $ sqlplus / as sysdba
--              SQL> @hf_latency_range_extended.sql
--
-- Notes:       Developed and tested on 19c
--
---------------------------------------------------------------------------------------
--
@@cs_internal/hf_primary.sql
@@cs_internal/hf_cdb_warn.sql
@@cs_internal/hf_set.sql
@@cs_internal/hf_def.sql
@@cs_internal/hf_file_prefix.sql
--
DEF cs_script_name = 'cs_latency_range_extended';
DEF cs_script_acronym = 'lre.sql | ';
DEF cs_hours_range_default = '12';
--
@@cs_internal/hf_sample_time_from_and_to.sql
@@cs_internal/hf_snap_id_from_and_to.sql
--
DEF cs_top_latency = '40';
DEF cs_top_load = '20';
DEF cs_ms_threshold_latency = '0.05';
DEF cs_aas_threshold_latency = '0.005';
DEF cs_aas_threshold_load = '0.05';
DEF cs_uncommon_col = 'PRINT';
DEF cs_execs_delta_h = '&&cs_from_to_seconds. secs';
-- DEF cs_execs_delta_h = '';
--
SELECT '&&cs_file_prefix._&&cs_script_name.' cs_file_name FROM DUAL;
--
-- @@cs_internal/&&cs_set_container_to_cdb_root.
--
@@cs_internal/hf_spool_head.sql
PRO SQL> @&&cs_script_name..sql "&&cs_sample_time_from." "&&cs_sample_time_to." 
@@cs_internal/hf_spool_id.sql
--
@@cs_internal/hf_spool_id_sample_time.sql
--
@@cs_internal/hf_latency_internal_cols.sql
@@cs_internal/hf_latency_internal_query_4.sql
@@cs_internal/hf_latency_internal_foot.sql
--
PRO
PRO SQL> @&&cs_script_name..sql "&&cs_sample_time_from." "&&cs_sample_time_to." 
--
@@cs_internal/hf_spool_tail.sql
--
-- @@cs_internal/&&cs_set_container_to_curr_pdb.
--
@@cs_internal/hf_undef.sql
@@cs_internal/hf_reset.sql
--