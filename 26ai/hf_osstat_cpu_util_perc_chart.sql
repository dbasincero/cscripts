--==========================================================================
-- Script    : hf_osstat_cpu_util_perc_chart.sql
-- Versao    : Oracle Database 26ai (26.x)
-- Uso       : Gráfico de Percentual de Utilização de CPU (AWR) -
--             Granularidade 15m
...
----------------------------------------------------------------------------------------
--
-- File name:   hf_osstat_cpu_util_perc_chart.sql
--
-- Purpose:     CPU Utilization Percent Chart (AWR) - 15m Granularity 
--
...
--              SQL> @hf_osstat_cpu_util_perc_chart.sql
--
...
--
DEF cs_script_name = 'cs_osstat_cpu_util_perc_chart';
...
SELECT s.end_interval_time,
      o.stat_name,
      CASE
        WHEN o.stat_name = 'NUM_CPUS' THEN o.value
        WHEN o.stat_name IN ('BUSY_TIME', 'IDLE_TIME') THEN o.value - LAG(o.value) OVER (PARTITION BY o.stat_name ORDER BY s.end_interval_time) 
      END AS value
  FROM dba_hist_osstat o, 
      dba_hist_snapshot s
WHERE s.dbid = (SELECT dbid FROM v$database)
  AND s.end_interval_time BETWEEN TO_DATE('&&cs_sample_time_from.', '&&cs_datetime_full_format.') - (2/24) AND TO_DATE('&&cs_sample_time_to.', '&&cs_datetime_full_format.')
  AND o.snap_id = s.snap_id
  AND o.dbid = s.dbid
  AND o.instance_number = s.instance_number
  AND o.stat_name IN ('BUSY_TIME', 'IDLE_TIME', 'NUM_CPUS')
),
