...
-- Seguranca  : executa comando de SO (host/!) — confira o comando antes de rodar; gera arquivo via SPOOL
-- Pre-req    : conexao com privilegio de DBA (acessa views V$/DBA_/CDB_).
--==========================================================================
----------------------------------------------------------------------------------------
--
-- File name:   pr.sql | hf_pr.sql 
--
...
--              3. one query
--
-- Examples:    1. SQL> @hf_pr.sql ""
--
--              2. SQL> @hf_pr.sql "/tmp/carlos.sql"
--
--              3. SQL> @hf_pr.sql "SELECT * FROM v$database;"
--
-- Notes:       When passing a query which contains single quotes then double them
--              e.g. SQL> @hf_pr.sql "SELECT sid||'',''||serial# AS sid_serial FROM v$session"
--
...
SAVE "/tmp/hf_pr_last_executed.sql" REPLACE;
STORE SET "/tmp/hf_pr_set_config.sql" REPLACE;
--
...
SELECT CASE 
       ...
       END AS cs_option,
       CASE 
         WHEN q'[&&cs_input_parameter.]' IS NULL THEN '/tmp/hf_pr_last_executed.sql' -- null
...
/
-- stores inline query
SPOOL "/tmp/hf_pr_inline_query.sql";
PRO &&cs_parameter.
SPOOL OFF;
--
COL script_name NEW_V script_name NOPRI;
SELECT CASE '&&cs_option.'
       WHEN '1' THEN '/tmp/hf_pr_last_executed.sql' -- null
       WHEN '2' THEN q'[&&cs_parameter.]' -- directory_path/script_name.sql
       WHEN '3' THEN '/tmp/hf_pr_inline_query.sql' -- query
       END AS script_name
  FROM DUAL
/
--
...
PRO
PRO
@"/tmp/hf_pr_set_config.sql";
HOST rm "/tmp/hf_pr_set_config.sql" "/tmp/hf_pr_last_executed.sql" "/tmp/hf_pr_inline_query.sql";
SET TERM ON;
--
-- from hf_ash_sample_detail.sql and hf_blocked_sessions_report.sql
