--==========================================================================
-- Script    : hf_sessions_age_by_machine_chart.sql
-- Versao    : Oracle Database 26ai (26.x)
-- Uso       : Idade de Sessão por Máquina (gráfico de série temporal)
-- Variaveis  : posicionais &3, &4, &5, &6; &&cs_sample_time_from, &&cs_sample_time_to
-- Pre-req    : conexao com privilegio de DBA (acessa views V$/DBA_/CDB_).
--==========================================================================
----------------------------------------------------------------------------------------
--
-- File name:   hf_sessions_age_by_machine_chart.sql
--
-- Purpose:     Session Age by Machine (time series chart)
--
...
--              SQL> @hf_sessions_age_by_machine_chart.sql
--
...
--
DEF cs_script_name = 'cs_sessions_age_by_machine_chart';
...
SELECT CASE WHEN UPPER(TRIM(NVL('&&cs2_type.', 'USER'))) IN ('USER', 'BACKGROUND', 'RECURSIVE', '*') THEN UPPER(TRIM(NVL('&&cs2_type.', 'USER'))) ELSE 'USER' END AS cs2_type FROM DUAL
/
PRO
PRO 4. Status (opt): [{INACTIVE}|ACTIVE|*|KILLED|CACHED|SNIPED]
DEF cs2_status = '&4.';
UNDEF 4;
COL cs2_status NEW_V cs2_status NOPRI;
SELECT CASE WHEN UPPER(TRIM('&&cs2_status.')) IN ('ACTIVE', 'INACTIVE', 'KILLED', 'CACHED', 'SNIPED', '*') THEN UPPER(TRIM('&&cs2_status.')) ELSE 'INACTIVE' END AS cs2_status FROM DUAL
/
--
...
/
PRO
PRO 6. Metric [{AVG}|MAX]:
DEF cs2_metric = '&6.';
UNDEF 6;
COL cs2_metric NEW_V cs2_metric NOPRI;
SELECT CASE WHEN UPPER(TRIM('&&cs2_metric.')) IN ('AVG', 'MAX') THEN UPPER(TRIM('&&cs2_metric.')) ELSE 'AVG' END AS cs2_metric FROM DUAL
/
--
DEF spool_id_chart_footer_script = 'hf_sessions_by_machine_footer.sql';
COL rn FOR 999;
DEF series_01 = ' ';
DEF series_02 = ' ';
DEF series_03 = ' ';
...
SELECT COALESCE(machine, '"null"') AS machine,
       ROW_NUMBER() OVER(ORDER BY &&cs2_metric.(last_call_et) DESC) AS rn
  FROM &&cs_tools_schema..iod_session
 WHERE snap_time BETWEEN TO_DATE('&&cs_sample_time_from.', '&&cs_datetime_full_format.') AND TO_DATE('&&cs_sample_time_to.', '&&cs_datetime_full_format.')
...
DEF vaxis_title = '&&cs2_metric. of Last Call Elapsed Time (Seconds)';
DEF xaxis_title = 'Status:"&&cs2_status." Type:"&&cs2_type." Machine:"&&cs2_machine." Metric:"&&cs2_metric."';
--
...
DEF report_foot_note = 'SQL> @&&cs_script_name..sql "&&cs_sample_time_from." "&&cs_sample_time_to." "&&cs2_type." "&&cs2_status." "&&cs2_machine." "&&cs2_metric."';
--
...
my_query AS (
SELECT time,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_01.' THEN last_call_et END) AS sessions_01,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_02.' THEN last_call_et END) AS sessions_02,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_03.' THEN last_call_et END) AS sessions_03,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_04.' THEN last_call_et END) AS sessions_04,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_05.' THEN last_call_et END) AS sessions_05,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_06.' THEN last_call_et END) AS sessions_06,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_07.' THEN last_call_et END) AS sessions_07,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_08.' THEN last_call_et END) AS sessions_08,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_09.' THEN last_call_et END) AS sessions_09,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_10.' THEN last_call_et END) AS sessions_10,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_11.' THEN last_call_et END) AS sessions_11,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_12.' THEN last_call_et END) AS sessions_12,
       &&cs2_metric.(CASE WHEN dimension_group = '&&series_13.' THEN last_call_et END) AS sessions_13
  FROM all_sessions
 GROUP BY
       time
)
