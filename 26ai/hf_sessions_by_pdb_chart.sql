--==========================================================================
-- Script    : hf_sessions_by_pdb_chart.sql
-- Versao    : Oracle Database 26ai (26.x)
-- Uso       : Sessões por PDB (gráfico de série temporal)
-- Variaveis  : posicionais &3, &4, &5; &&cs_sample_time_from, &&cs_sample_time_to
-- Pre-req    : conexao com privilegio de DBA (acessa views V$/DBA_/CDB_).
--==========================================================================
----------------------------------------------------------------------------------------
--
-- File name:   hf_sessions_by_pdb_chart.sql
--
-- Purpose:     Sessions by PDB (time series chart)
--
...
--              SQL> @hf_sessions_by_pdb_chart.sql
--
...
--
DEF cs_script_name = 'cs_sessions_by_pdb_chart';
...
WITH
by_pdb AS (
SELECT h.con_id||' '||c.name AS pdb,
       ROW_NUMBER() OVER(ORDER BY COUNT(*) DESC) AS rn
  FROM &&cs_tools_schema..iod_session h, v$containers c
 WHERE h.snap_time BETWEEN TO_DATE('&&cs_sample_time_from.', '&&cs_datetime_full_format.') AND TO_DATE('&&cs_sample_time_to.', '&&cs_datetime_full_format.')
   AND ('&&cs2_machine.' IS NULL OR COALESCE(h.machine, '"null"') LIKE CHR(37)||'&&cs2_machine.'||CHR(37))
   AND '&&cs_con_id' IN ('1', h.con_id)
   AND '&&cs2_type.' IN (h.type, '*')
   AND '&&cs2_status.' IN (h.status, '*')
...
SELECT /*+ MATERIALIZE NO_MERGE */
  ...
  FROM by_pdb a, max_top
 WHERE a.rn >= max_top.max_rn
 GROUP BY
       max_top.max_rn
),
top_and_bottom AS (
SELECT rn, pdb AS dimension_group
  FROM top
 UNION ALL
...
--
DEF report_title = 'Sessions by PDB between &&cs_sample_time_from. and &&cs_sample_time_to. UTC';
