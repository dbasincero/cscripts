--==========================================================================
-- Script    : hf_spbl_create.sql
-- Versao    : Oracle Database 26ai (26.x)
-- Uso       : Cria um SQL Plan Baseline para um dado SQL_ID
-- Variaveis  : posicionais &1, &2; &&cs_dbid, &&cs_plan_hash_value, &&cs_sql_id
-- Pre-req    : conexao com privilegio de DBA (acessa views V$/DBA_/CDB_).
--==========================================================================
----------------------------------------------------------------------------------------
--
-- File name:   hf_spbl_create.sql
--
...
@@hf_internal/&&cs_spbl_create_pre.
--
---------------------------------------------------------------------------------------
--
...
BEGIN
  IF TO_NUMBER('&&cs_plan_hash_value.') > 0 THEN
    ...
    LOOP
      l_description := 'hf_spbl_create.sql SRC=MEM SQL_ID=&&cs_sql_id. PHV=&&cs_plan_hash_value. &&cs_reference_sanitized. &&who_am_i. CREATED=&&creation_time.';
    ...
    IF l_end_snap > l_begin_snap THEN
      ...
      -- $THEN
        ...
        IF l_created_plans > 0 THEN
          ...
          LOOP
            l_description := 'hf_spbl_create.sql SRC=STS SQL_ID=&&cs_sql_id. PHV=&&cs_plan_hash_value. &&cs_reference_sanitized. &&who_am_i. CREATED=&&creation_time.';
            l_modified_plans := DBMS_SPM.alter_sql_plan_baseline(sql_handle => i.sql_handle, plan_name => i.plan_name, attribute_name  => 'DESCRIPTION', attribute_value => l_description);
      ...
      --   LOOP
      --     l_description := 'hf_spbl_create.sql SRC=AWR SQL_ID=&&cs_sql_id. PHV=&&cs_plan_hash_value. &&cs_reference_sanitized. &&who_am_i. CREATED=&&creation_time.';
      --     l_modified_plans := DBMS_SPM.alter_sql_plan_baseline(sql_handle => i.sql_handle, plan_name => i.plan_name, attribute_name  => 'DESCRIPTION', attribute_value => l_description);
...
/
--
@@hf_internal/&&cs_spbl_create_post.
--
---------------------------------------------------------------------------------------
--
SET SERVEROUT OFF;
