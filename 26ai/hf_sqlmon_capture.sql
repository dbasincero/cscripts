--==========================================================================
-- Script    : hf_sqlmon_capture.sql
-- Versao    : Oracle Database 26ai (26.x)
-- Uso       : Gera Relatórios SQL Monitor para um dado SQL_ID por um curto
--             período de tempo
-- Variaveis  : posicionais &1, &2, &3; &&cs_signature, &&cs_sql_id
-- Seguranca  : gera arquivo via SPOOL
-- Pre-req    : conexao com privilegio de DBA (acessa views V$/DBA_/CDB_).
--==========================================================================
----------------------------------------------------------------------------------------
--
-- File name:   hf_sqlmon_capture.sql
--
...
BEGIN
  ...
  $THEN
    DBMS_SQLDIAG_INTERNAL.i_create_patch(sql_id => '&&cs_sql_id.', hint_text => q'[&&cbo_hints. &&cs_additional_cbo_hints.]', name => 'spch_&&cs_sql_id._&&hash.', description => q'[hf_sqlmon_capture.sql /*+ &&cbo_hints. &&cs_additional_cbo_hints. */ &&cs_reference_sanitized. &&who_am_i.]'); -- 12c
  $ELSE
    l_name := DBMS_SQLDIAG.create_sql_patch(sql_id => '&&cs_sql_id.',  hint_text => q'[&&cbo_hints. &&cs_additional_cbo_hints.]', name => 'spch_&&cs_sql_id._&&hash.', description => q'[hf_sqlmon_capture.sql /*+ &&cbo_hints. &&cs_additional_cbo_hints. */ &&cs_reference_sanitized. &&who_am_i.]'); -- 19c
  $END
  NULL;
END;
/
