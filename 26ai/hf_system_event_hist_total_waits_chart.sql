--==========================================================================
-- Script    : hf_system_event_hist_total_waits_chart.sql
-- Versao    : Oracle Database 26ai (26.x)
-- Uso       : Subconjunto de Total Waits de System Event a partir do AWR
--             (gráfico de série temporal)
-- Variaveis  : posicionais &3, &4, &5, &6, &7, &8, &9, &10; &&cs_dbid, &&cs_sample_time_from, &&cs_sample_time_to
-- Pre-req    : conexao com privilegio de DBA (acessa views V$/DBA_/CDB_).
--==========================================================================
----------------------------------------------------------------------------------------
--
-- File name:   hf_system_event_hist_total_waits_chart.sql
--
-- Purpose:     Subset of System Event Total Waits from AWR (time series chart)
--
...
--              SQL> @hf_system_event_hist_total_waits_chart.sql
--
...
--
DEF cs_script_name = 'cs_system_event_hist_total_waits_chart';
...
--
DEF report_title = "System Event History - Total Waits";
DEF chart_title = "System Event History - Total Waits";
...
SELECT /*+ MATERIALIZE NO_MERGE */
       ...
       SUM(CASE WHEN event_name = '&&evnt_nam_1.' THEN h.total_waits ELSE 0 END) total_waits_1,
       SUM(CASE WHEN event_name = '&&evnt_nam_2.' THEN h.total_waits ELSE 0 END) total_waits_2,
       SUM(CASE WHEN event_name = '&&evnt_nam_3.' THEN h.total_waits ELSE 0 END) total_waits_3,
       SUM(CASE WHEN event_name = '&&evnt_nam_4.' THEN h.total_waits ELSE 0 END) total_waits_4,
       SUM(CASE WHEN event_name = '&&evnt_nam_5.' THEN h.total_waits ELSE 0 END) total_waits_5,
       SUM(CASE WHEN event_name = '&&evnt_nam_6.' THEN h.total_waits ELSE 0 END) total_waits_6
...
SELECT ', [new Date('||
       ...
       ')'||
       ','||num_format(q.total_waits_1, 3)|| 
       ','||num_format(q.total_waits_2, 3)|| 
       ','||num_format(q.total_waits_3, 3)|| 
       ','||num_format(q.total_waits_4, 3)|| 
       ','||num_format(q.total_waits_5, 3)|| 
       ','||num_format(q.total_waits_6, 3)|| 
       ']'
  FROM my_query q
 ORDER BY
       q.time
